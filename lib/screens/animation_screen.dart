        ...
        builder: (context, provider, child) {
          return Row(
            children: [
              ...
              if (_showSettings)
                Container(
                  width: 450, // Increased from 300 by 150px
  ...
  Widget _buildActionButtons(ConversionProvider provider) {
    return Row(
      children: [
        ...
        Expanded(
          child: NeumorphicButton(
            onPressed: provider.isConverting
                ...
                : (provider.canConvert ? () => _startConversion(provider) : null),
            isLoading: provider.isConverting && !provider.isCancelled,
            child: Padding(
              padding: const EdgeInsets.symmetric(vertical: 12.0),
              child: Text(
                provider.isConverting 
                    ? (provider.isCancelled ? 'Cancelling...' : 'Cancel')
                    : 'Convert to WebP',
                style: Theme.of(
                  context,
                ).textTheme.bodyMedium?.copyWith(
                  fontWeight: FontWeight.w600,
                  color: provider.isConverting ? Colors.red : null,
                ),
              ),
            ),
          ),
        ...
        // Save button
        if (provider.hasOutput && !provider.isConverting)
          NeumorphicButton(
            onPressed: () => _saveFile(provider),
            child: const Padding(
